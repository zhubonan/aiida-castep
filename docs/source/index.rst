...
As an example, let's say we want to compute the band structure of silicon. We should first do a
full relaxation of the structure including the cell parameters.
Then, the output geometry can be used as the input of the band structure calculation.
In practice, the model of the first CASTEP calculation should also be used (via ``continuation: first_calc.check``).
Below is a graph representation for this simple two-step process generated by AiiDA:
