...
def param_dict():
    ...
    out = {
        "PARAM": {"task": "singlepoints", "xc_functional": "lda"},
        "CELL": {"fix_all_cell": True, "cell_constraints": ["1 1 1", "2 2 3"]},
    }
    return out
...
def test_inp_gen_cell(gen_instance, sto_calc_inputs):
    ...
    gen_instance._include_extra_kpoints(
        kpn1, "phonon", {"task": ("phonon",), "need_weights": False}
    )
    ...
    gen_instance._include_extra_kpoints(
        kpn2, "bs", {"task": ("bandstructure",), "need_weights": True}
    )
    ...
    # No weights
    gen_instance._include_extra_kpoints(
        kpn2, "bs", {"task": ("bandstructure",), "need_weights": False}
    )
...
def test_cell_with_spin(gen_instance, sto_calc_inputs):
    ...
    sto_calc_inputs.settings = orm.Dict(dict={"SPINS": [[1.0, 1.0, 1.0]] * 5})
    gen_instance.prepare_inputs()
    positions = gen_instance.cell_file["POSITIONS_ABS"]
    for i in range(5):
...
def test_submission(clear_database_before_test, sto_calc_inputs, sto_spectral_inputs):
    """
...
def test_submit_test_function(clear_database_before_test, sto_calc_inputs):
    ...
    from aiida_castep.calculations.castep import (
        CastepCalculation,
        submit_test,
    )
