...
def test_parse_warnings(
    ...
    content.append("SCF cycles performed but system has not reached the groundstate")
    node = generate_calc_job_node(
        "castep.castep",
        "H2-geom",
        inputs=h2_calc_inputs,
    ...
    node = generate_calc_job_node(
        ...
        outfile_override={
            "aiida.castep": None
        },  # Remove the 'aiida.castep' output file
    )
...
def test_parser_retrieved(
    db_test_app, output_folder, generate_parser, generate_calc_job_node, sto_calc_inputs
):
    """
    ...
    md_keys = ["hamilt_energy", "kinetic_energy", "velocities", "temperatures", "times"]
...
def test_bands_from_castepbin(
    db_test_app, generate_parser, data_path, generate_calc_job_node, sto_calc_inputs
):
    """
    Iterate through internal test cases.
    Check if the results are parsed correctly.
