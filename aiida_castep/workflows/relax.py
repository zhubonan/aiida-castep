...
class CastepRelaxWorkChain(WorkChain):
    ...
    def define(cls, spec):
        ...
        super().define(spec)
        spec.expose_inputs(CastepBaseWorkChain, namespace="base", exclude=("calc",))
        spec.expose_inputs(
    ...
    def setup(self):
        ...
        assert restart_mode in [
            ...
            "continuation",
            "structure",
        ], f"Invalid restart mode: {restart_mode}"
    ...
    def should_run_relax(self):
        ...
        return (
            ...
            and self.ctx.iteration < self.ctx.max_meta_iterations
        )

    def run_relax(self):
        """Run the relaxation"""
        self.ctx.iteration += 1
        link_label = f"iteration_{self.ctx.iteration}"
        # Assemble the inputs
        ...
        self.report(
            "launching CastepBaseWorkChain<{}> Iteration #{}".format(
    ...
    def inspect_relax(self):
        ...
        if not workchain.is_finished_ok:
            self.report(
                "Relaxation CastepBaseWorkChain failed with exit status {}".format(
        ...
        if output_parameters.get("geom_unconverged") is True:
            ...
            if self.ctx.restart_mode in ["reuse", "continuation"]:
                ...
                self.ctx.base_update[
                    f"{self.ctx.restart_mode}_folder"
                ] = workchain.outputs.remote_folder
    ...
    def finalize(self):
        ...
        try:
            ...
            self.report(
                "Exception occurred during the cleaning of the remote contents: {}".format(
                    exception.args
                )
            )
    ...
    def _push_parameters(self, workchain):
        ...
        query.append(orm.WorkChainNode, filters={"id": workchain.pk}, tag="work")
        query.append(
            orm.Dict,
            with_incoming="work",
            tag="output_dict",
        ...
        if orig_in_param != last_param:
            ...
            self.ctx.calc_update[IN_LINKS["parameters"]] = orm.Dict(dict=last_param)
        return None
...
class CastepAlterRelaxWorkChain(CastepRelaxWorkChain):
    ...
    def define(cls, spec):
        ...
        spec.exit_code(
            201, "ERROR_NO_CELL_CONS_SET", "NO cell_constraints find in the input"
        )
    ...
    def inspect_relax(self):
        ...
        if unconv is True or self.ctx.is_fixed_cell:
            ...
            if self.ctx.is_fixed_cell:
                self.set_cons_and_imax(
                    self.ctx.init_cell_cons, self.ctx.var_cell_iter_max
                )
                self.ctx.is_fixed_cell = False
                ...
                self.set_cons_and_imax(["0 0 0", "0 0 0"], self.ctx.fix_cell_iter_max)
