    ...
    def create_restart(
        ...
        **kwargs,
        ...
        if self._node.exit_status != 0 and not ignore_state:
            raise RuntimeError("exit_status is not 0. Set ignore_state to ignore")
        ...
        if use_output_structure is True:
            builder[INPUT_LINKNAMES["structure"]] = self._node.outputs.__getattr__(
        ...
        if restart_mode == "continuation" or kwargs.get("reuse"):
            ...
            ] = self._node.outputs.__getattr__("remote_folder")
...
def castep_input_summary(calc):
    ...
    out_info["custom_scheduler_commands"] = options.get("custom_scheduler_commands")
    ...
    out_info["description"] = (
        calc.description if is_node else metadata.get("description")
    )

    # Show the parent calculation whose RemoteData is linked to the node
    if in_remote is not None:
        input_calc = [n.node for n in in_remote.get_incoming(link_type=LinkType.CREATE)]
        assert (
            len(input_calc) < 2
        ), "More than one JobCalculation found, something seriously wrong"
        if input_calc:
            input_calc = input_calc[0]
            out_info["parent_calc"] = {"pk": input_calc.pk, "label": input_calc.label}
