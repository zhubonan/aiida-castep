...
class CastepInputGenerator:
    ...
    def prepare_inputs(self, reset=True):
        ...
        param_dict = {k: _lowercase_dict(v, dict_name=k) for k, v in param_dict.items()}
    ...
    def _prepare_cell_file(self):
        ...
        for vector in self.inputs[in_ln["structure"]].cell:
            cell_vector_list.append("{:18.10f} {:18.10f} " "{:18.10f}".format(*vector))
            ...
            if param_spin is not None:
                ...
                if param_spin != total_spin:
                    raise InputValidationError(
                        ...
                        "Total spin: {} in cell file but {} in param file".format(
                            total_spin, param_spin
                        )
                    )
            else:
        ...
        if not kpoints and use_kpoints:
            raise InputValidationError("Kpoints required but not found in the input")
        ...
        if self.inputs.metadata.options.use_kpoints:
            ...
            if has_mesh is True:
                ...
                if offset != [0.0, 0.0, 0.0]:
                    self.cell_file["kpoints_mp_offset"] = "{} {} {}".format(*offset)
            else:
                for kpoint, weight in zip(kpoints_list, weights):
                    kpoints_line_list.append(
                        "{:18.10f} {:18.10f} "
    ...
    def _include_extra_kpoints(self, kpn_node, kpn_name, kpn_settings, report_fn=None):
        ...
        try:
            ...
            try:
                ...
                has_mesh = False
                if num_kpoints == 0:
                    raise InputValidationError("At least one k points must be provided")
            except AttributeError:
                raise InputValidationError(
            ...
            try:
                ...
                if kpn_settings["need_weights"] is True:
                    ...
                    if report_fn is not None:
                        report_fn(
                            "Warning:filling evenly distributed weights for {}_kpoints".format(
                                kpn_name
                            )
                        )
        ...
        if has_mesh is True:
            mesh_name = f"{kpn_name}_kpoint_mp_grid"
            self.cell_file[mesh_name] = "{} {} {}".format(*mesh)
            if offset != [0.0, 0.0, 0.0]:
                self.cell_file[mesh_name.replace("grid", "offset")] = "{} {} {}".format(
                    *offset
                )
        else:
            extra_kpts_lines = []
            ...
            bname = f"{kpn_name}_kpoint_list".upper()
            ...
            for symbol in symbols:
                ...
                if isinstance(ps_node, OTFGData):
                    ...
                    try:
                        ...
                        raise InputValidationError(
                            "Unknown node as pseudo: {}. Exception raised: {}".format(
                                ps_node, error
                            )
                        )
