...
class CastepHelper(object):
    ...
    def load_helper_dict(self):
        ...
        """
        fpath_ver = find_help_info()
        path = None
        # Select the first one found or a matched one if applicable
        for p, v in fpath_ver:
            if self.version is None or v == self.version:
                path = p
                break
...
def find_help_info():
    ...
    """
    home = os.getenv("HOME")
    fnames = ('.castep_help_info_*.json', 'castep_help_info_*.json')
    basedirs = (module_path, home)
    # Make sure prefer those at $HOME
    patterns = [os.path.join(a, b) for b in fnames for a in basedirs]
    files_paths = []
    for pattern in patterns:
        files_paths.extend(glob(pattern))
    res = []

    for fn in files_paths:
        fname = os.path.split(fn)[1]
        tmp = fname.replace(".json", "").split("_")
        ver = tmp[-1] if len(tmp) == 4 else None
        res.append((fn, ver))
    # Sort with the version, in numerical descending order
    res.sort(key=lambda x: float(x[1]) if x is not None else None,
             reverse=True)
    return res
