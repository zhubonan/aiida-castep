...
def mock_castep(seed):
    ...
    mock_output.append(
        "MOCK PREPEND: AIIDA_PATH: " + os.environ.get("AIIDA_PATH", "") + "\n"
    )
    ...
    mock_registry_path = os.environ.get("MOCK_CODE_BASE", data_path("registry"))

    click.echo(f"DEBUG: MOCK REGSISTRY PATH = {str(mock_registry_path)}")
    registry = MockRegistry(mock_registry_path)
    mock = MockCastep(pwd, registry, seed)
    if mock.is_runnable:
        detected_path = mock.registry.get_path_by_hash(registry.compute_hash(pwd))
        mock_output.append(
